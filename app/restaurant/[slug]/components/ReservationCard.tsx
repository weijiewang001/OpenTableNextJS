...
export default function ReservationCard(
  ...
  const { data, loading, error, fetchAvailablilites } = useAvailabilities();
      ...
      <div className="fixed w-[15%] bg-white rounded p-3 shadow">
        ...
        <div className="flex justify-between">
          <div className="flex flex-col w-[48%]">
            ...
            <ReactDatePicker selected={selectedDate} onChange={(selectedDate) => handleChangeData(selectedDate)} className="py-3 border-b font-light text-reg w-24" dateFormat="MMMM d" wrapperClassName="w-[48%]"></ReactDatePicker>
          </div>
          <div className="flex flex-col w-[48%]">
            <label htmlFor="">Time</label>
            <select name="" id="" className="py-3 border-b font-light" value={time} onChange={e => setTime(e.target.value)}>
              {filterTimeByRestaurantOpenWindow().map(time => (<option key={time.time} value={time.time}>{time.displayTime}</option>))}
            </select>
          </div>
        </div>
        <div className="mt-5">
